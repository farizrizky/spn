                ...
                @foreach ($product as $p)
                    <div class="col-lg-4 col-md-6 wow fadeIn" data-wow-delay=".2s">
                        <div class="single-service">
                            ...
                            @endif
                            <h5 class="title text-center"><a href="{{ route('web.product-detail', ['slug' => $p->slug]) }}">{{ $p->name }}</a></h5>
                            <p class="mt-10 text-center">{{ Str::limit(strip_tags($p->description), 120) }}</p>
                            <div class="d-flex justify-content-center align-items-center mb-30">
                                <a href="{{ route('web.product-detail', ['slug' => $p->slug]) }}" class="primary-btn mt-20"><span class="text">Selengkapnya</span> <span class="icon"><i class="ri-arrow-right-double-line"></i></span></a>
                            </div>
                        </div>                    
                    </div>
                @endforeach
